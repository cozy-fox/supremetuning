...
import { createAuditLog } from './backup-service';
...
export async function initDataLayer() {
  try {
    ...
    const groupsCol = await getCollection(COLLECTIONS.groups);
    await groupsCol.createIndex({ id: 1 }, { unique: true });
    await groupsCol.createIndex({ brandId: 1 });
    await groupsCol.createIndex({ brandId: 1, order: 1 });
    ...
    await modelsCol.createIndex({ groupId: 1 });
    ...
    console.log(`📦 MongoDB connected: ${brandCount} brands, ${groupCount} groups found`);
...
/**
 * Update a single document by ID (with audit logging)
 */
export async function updateById(collectionName, id, updateData, options = {}) {
  try {
    const collection = await getCollection(collectionName);
    const parsedId = parseInt(id);

    // Get document before update for audit log
    const before = await collection.findOne({ id: parsedId });
    if (!before) {
    ...
    const after = await collection.findOne({ id: parsedId });

    // Create audit log (skip for system collections)
    if (!['backups', 'metadata', 'audit_logs', 'full_backups'].includes(collectionName)) {
      ...
      }).catch(err => console.error('⚠️ Audit log failed:', err));
    }
...
/**
 * Delete a single document by ID (with audit logging)
 */
export async function deleteById(collectionName, id, options = {}) {
  try {
    const collection = await getCollection(collectionName);
    const parsedId = parseInt(id);
...
/**
 * Insert a new document (with audit logging)
 */
export async function insertOne(collectionName, data, options = {}) {
  try {
    const collection = await getCollection(collectionName);
...
export async function saveData(newData) {
  ...
  try {
    ...
    await Promise.all([
      ...
      (await getCollection(COLLECTIONS.groups)).deleteMany({}),
    ...
    if (validatedData.groups.length > 0) {
      await (await getCollection(COLLECTIONS.groups)).insertMany(validatedData.groups);
    }
    ...
    console.log(`   - Groups: ${validatedData.groups.length}`);
...
export async function getData() {
  try {
    const [brands, groups, models, types, engines, stages] = await Promise.all([
      (await getCollection(COLLECTIONS.brands)).find({}).sort({ name: 1 }).toArray(),
      (await getCollection(COLLECTIONS.groups)).find({}).sort({ brandId: 1, isPerformance: 1, order: 1 }).toArray(),
    ...
    const cleanData = {
      ...
      groups: groups.map(({ _id, ...rest }) => rest),
...
/**
 * Get all groups (sorted by brandId, isPerformance, and order)
 * Standard groups (isPerformance: false) appear first
 */
export async function getGroups(brandId) {
  if (!brandId) {
    return findBy(COLLECTIONS.groups, {}, null, { sort: { brandId: 1, isPerformance: 1, order: 1 } });
  }
  return findBy(COLLECTIONS.groups, 'brandId', parseInt(brandId), { sort: { isPerformance: 1, order: 1 } });
}
...
export async function getGroupById(groupId) {
  return findById(COLLECTIONS.groups, groupId);
}

/**
 * Get a group by name and brandId
 */
export async function getGroupByName(brandId, nameOrSlug) {
  try {
    ...
    console.error('❌ getGroupByName error:', error);
    return null;
  }
}
...
export async function brandHasGroups(brandId) {
  try {
    ...
    console.error('❌ brandHasGroups error:', error);
    return false;
  }
}
...
/**
 * Get models by brandId or groupId (sorted alphabetically by name)
 */
export async function getModels(brandId, groupId) {
  if (!brandId && !groupId) {
    return findBy(COLLECTIONS.models, {}, null, { sort: { name: 1 } });
  }

  if (groupId) {
    return findBy(COLLECTIONS.models, 'groupId', parseInt(groupId), { sort: { name: 1 } });
  }
