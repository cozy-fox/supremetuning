...
export async function getData() {
  try {
    const [brands, groups, models, types, engines, stages] = await Promise.all([
      ...
      (await getCollection(COLLECTIONS.groups)).find({}).sort({ brandId: 1, order: 1 }).toArray(),
...
/**
 * Get all groups (sorted by brandId and order)
 */
export async function getGroups(brandId) {
  if (!brandId) {
    return findBy(COLLECTIONS.groups, {}, null, { sort: { brandId: 1, order: 1 } });
  }
  return findBy(COLLECTIONS.groups, 'brandId', parseInt(brandId), { sort: { order: 1 } });
}
